...
ggplot(digitos, aes(intensidade, simetria, color = factor(digito))) +
  geom_point(alpha = 0.5) +
  labs(x = "Intensidade", y = "Simetria", color = "Dígito") +
  theme_bw()
...
ggplot(digitos_tr,
       aes(intensidade, simetria, color = factor(digito))) +
  geom_point(alpha = 0.5) +
  labs(x = "Intensidade", y = "Simetria", color = "Dígito") +
  theme_bw()
...
ggplot(digitos_tr,
  ...
  labs(x = "Intensidade", y = "Densidade", fill = "Dígito") +
  theme_bw()
...
ggplot(digitos_tr,
  ...
  geom_density(alpha = 0.5) +
  labs(x = "Simetria", y = "Densidade", fill = "Dígito") +
  theme_bw()
...
(tabela_tr <- table(predito = pred_labels_tr, observado = digitos_tr$digito))

# precisao dentro da amostra
(prec_dentro <- (tabela_tr[1, 1] + tabela_tr[2, 2]) / nrow(digitos_tr) * 100)

# precisao dentro da amostra (calculo alternativo)
(prec_dentro <- mean(pred_labels_tr == digitos_tr$digito) * 100)
...
(tabela_tst <- table(predito = pred_labels_tst, observado = digitos_tst$digito))

# precisao fora da amostra
(prec_fora <- (tabela_tst[1, 1] + tabela_tst[2, 2]) / nrow(digitos_tst) * 100)
...
grid <- expand.grid(intensidade = seq(min(digitos_tr$intensidade),
...
grid$pred <- ifelse(predict(fit, grid, type = "response") > 0.5, 1, 0)

ggplot() +
  geom_tile(aes(intensidade, simetria, fill = factor(pred)),
            data = grid, alpha = 0.5, show.legend = FALSE) +
  geom_point(aes(intensidade, simetria, color = factor(digito)),
             data = digitos_tr) +
  labs(x = "Intensidade", y = "Simetria",
       color = "Dígito", title = "Regressão logística") +
  theme_bw()
...
# comparativo: observado vs predito (dentro da amostra)
(tabela_tr <- table(predito = class_nb_tr, observado = digitos_tr$digito))
...
# comparativo: observado vs predito (fora da amostra)
(tabela_tst <- table(predito = class_nb_tst, observado = digitos_tst$digito))

# precisao fora da amostra
(prec_fora <- (tabela_tst[1, 1] + tabela_tst[2, 2]) / nrow(digitos_tst) * 100)

resultados[resultados$modelo == "Naive Bayes", 2:3] <- c(prec_dentro, prec_fora)
resultados
...
ggplot() +
  ...
  geom_point(aes(intensidade, simetria, color = factor(digito)),
             data=digitos_tr) +
  labs(x = "Intensidade", y = "Simetria",
       color = "Dígito", title = "Naive Bayes") +
  theme_bw()
...
knn_tr <- data.frame(digito = digitos_tr$digito,
...
knn_tst <- data.frame(digito = digitos_tst$digito,
...
(prec_dentro <- (tabela_tr[1, 1] + tabela_tr[2, 2]) / nrow(knn_tr) * 100)

# predicao fora da amostra
set.seed(1)
knn_pred <- knn(knn_tr[,-1], knn_tst[,-1], knn_tr$digito, k = )
tabela_tst <- table(predito = knn_pred, observado = knn_tst$digito)
(prec_fora <- (tabela_tst[1, 1] + tabela_tst[2, 2]) / nrow(knn_tst) * 100)

resultados[resultados$modelo == "kNN", 2:3] <- c(prec_dentro, prec_fora)
resultados

# boundary
grid2 <- data.frame(intensidade = (grid$intensidade - min(digitos_tr$intensidade))/
                      (max(digitos_tr$intensidade) - min(digitos_tr$intensidade)),
...
ggplot() +
  geom_tile(aes(intensidade, simetria, fill = factor(pred)),
            data = grid2, alpha = 0.5, show.legend = FALSE) +
  geom_point(aes(intensidade, simetria, color = factor(digito)),
             data=knn_tr) +
  labs(x = "Intensidade", y = "Simetria",
       color = "Dígito", title = "Naive Bayes") +
...
# TAREFA: escolher o melhor valor de k
